...
fn ts_reexport_star_callers() {
    ...
    fs::write(repo.join("mod/agg.ts"), "export * from './real'\n").unwrap();
    fs::write(
        repo.join("main.ts"),
        "import { bar } from './mod/agg'\nfunction f(){ bar(); }\n",
    )
    .unwrap();
    git(&repo, &["add", "."]);
    git(&repo, &["commit", "-m", "init", "-q"]);
    fs::write(
        repo.join("mod/real.ts"),
        "export function bar(): void { const x=1; }\n",
    )
    .unwrap();
