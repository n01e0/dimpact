...
#[test]
fn js_module_exports_named_callers() {
    let dir = TempDir::new().unwrap();
    let repo = dir.path().to_path_buf();
    git(&repo, &["init", "-q"]);
    git(&repo, &["config", "user.email", "tester@example.com"]);
    git(&repo, &["config", "user.name", "Tester"]);
    fs::write(repo.join("mod.js"), "module.exports.foo = function(){}\n").unwrap();
    fs::write(
        repo.join("main.js"),
        "const M = require('./mod');\nfunction caller(){ M.foo(); }\n",
    )
    .unwrap();
    git(&repo, &["add", "."]);
    ...
    fs::write(
        repo.join("mod.js"),
        "module.exports.foo = function(){ const z=1; }\n",
    )
    .unwrap();
    ...
    assert!(
        out.contains("\"caller\""),
        "impact should include caller, got: {}",
        out
    );
}
