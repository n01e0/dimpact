...
fn ts_reexport_chain_callers() {
    ...
    fs::write(repo.join("mod/c.ts"), "export function x(): void {}\n").unwrap();
    fs::write(repo.join("mod/b.ts"), "export { x } from './c'\n").unwrap();
    fs::write(repo.join("mod/a.ts"), "export { x as y } from './b'\n").unwrap();
    fs::write(repo.join("main.ts"), "import { y } from './mod/a'\nfunction f(){ y(); }\n").unwrap();
    git(&repo, &["add", "."]);
    git(&repo, &["commit", "-m", "init", "-q"]);
    fs::write(repo.join("mod/c.ts"), "export function x(): void { const k=1; }\n").unwrap();
