...
fn cli_impact_risk_yaml_reports_fixture_summary() {
    ...
    assert_eq!(risk.get(&level_key).and_then(|value| value.as_str()), Some("medium"));
    assert_eq!(risk.get(&direct_key).and_then(|value| value.as_u64()), Some(1));
    assert_eq!(risk.get(&transitive_key).and_then(|value| value.as_u64()), Some(1));
    assert_eq!(risk.get(&files_key).and_then(|value| value.as_u64()), Some(1));
    assert_eq!(risk.get(&symbols_key).and_then(|value| value.as_u64()), Some(2));
}
