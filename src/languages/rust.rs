...
fn find_block_end(source: &str, start_line_idx: usize, open_brace_on_line: bool) -> usize {
    ...
    for (i, line) in source.lines().enumerate().skip(start_line_idx) {
        ...
        if open_brace_on_line && i == start_line_idx {
            // include brace on same line
            if !started {
                depth += 1;
                started = true;
            }
...
impl crate::languages::LanguageAnalyzer for RustAnalyzer {
    ...
    fn symbols_in_file(&self, path: &str, source: &str) -> Vec<Symbol> {
        let re_fn =
            Regex::new(r"^\s*(?:pub\s+)?(?:async\s+)?(?:const\s+)?fn\s+([a-zA-Z_][a-zA-Z0-9_]*)")
                .unwrap();
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if let Some(caps) = re_struct.captures(l) {
                let name = caps.get(1).unwrap().as_str();
                let end_idx = if l.contains('{') {
                    find_block_end(source, idx, true)
                } else {
                    idx
                };
                symbols.push(mk_symbol(
    ...
    fn imports_in_file(
        ...
        fn flatten(items: &str, prefix: &str, out: &mut std::collections::HashMap<String, String>) {
            ...
            let push_item =
                |tok: &str, prefix: &str, out: &mut std::collections::HashMap<String, String>| {
                    let tok = tok.trim();
                    if tok.is_empty() {
                        return;
                    }
                    ...
                    if let Some(brace) = tok.find('{') {
                        let pfx =
                            format!("{}::{}", prefix, tok[..brace].trim().trim_end_matches("::"));
                        let inner = tok[brace + 1..].trim_end_matches('}');
                        flatten(inner, &pfx, out);
                        return;
                    }
                    let (name, alias) = if let Some((n, a)) = tok.split_once(" as ") {
                        (n.trim(), a.trim())
                    } else {
                        (tok, tok)
                    };
        ...
        for mut line in source.lines().map(|l| l.trim()) {
            ...
            if let Some(stripped) = line.strip_prefix("pub use ") {
                line = stripped;
            } else if let Some(stripped) = line.strip_prefix("use ") {
                line = stripped;
            }
            ...
            if let Some(brace_pos) = line.find('{') {
                ...
                let (path_spec, alias) = if let Some((p, a)) = line.split_once(" as ") {
                    (p.trim(), a.trim())
                } else {
                    (line, line.split("::").last().unwrap_or(line))
                };
