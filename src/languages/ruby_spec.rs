        ...
        for caps in self.runner.run_captures(source, &self.queries.decl) {
            if let Some(nc) = caps.iter().find(|c| c.name == "name") {
                ...
                if let Some(decl_node) = caps.iter().find(|c| c.name == "decl") {
                    ...
                    for c in &caps {
                        match c.kind.as_str() {
                            "class" => { kind = SymbolKind::Struct; break; }
                            "module" => { kind = SymbolKind::Module; break; }
                            "method" | "singleton_method" => { kind = SymbolKind::Method; break; }
                            _ => {}
                        }
                    }
                }
                ...
                out.push(Symbol { id: SymbolId::new("ruby", path, &kind, name, sl), name: name.to_string(), kind, file: path.to_string(), range: TextRange { start_line: sl, end_line: el }, language: "ruby".to_string() });
            }
        }
        out
    }
    ...
    fn unresolved_refs(&self, path: &str, source: &str) -> Vec<UnresolvedRef> {
        ...
        for (i, line) in source.lines().enumerate() {
            if let Some(cap) = re_bare.captures(line) {
                ...
                if rest_trim.starts_with('=') || rest_trim.starts_with('.') || rest_trim.starts_with("::") || rest_trim.starts_with('(') {
                    // likely assignment, receiver call, namespace, or explicit paren-call handled elsewhere
                } else {
                    let ln = (i as u32) + 1;
                    if !seen.contains(&(ln, name.to_string())) {
                        out.push(UnresolvedRef { name: name.to_string(), kind: RefKind::Call, file: path.to_string(), line: ln, qualifier: None, is_method: true });
                    }
                }
            }
        }
    ...
    fn imports_in_file(&self, path: &str, source: &str) -> std::collections::HashMap<String, String> {
        ...
        for line in source.lines() {
            if let Some(cap) = re_req.captures(line) {
                ...
                if pfx.ends_with(".rb") { pfx = &pfx[..pfx.len()-3]; }
                ...
                let normalized = if kind == "require_relative" {
                    ...
                    canon.trim_start_matches("./").trim_start_matches('.').trim_start_matches('/') .to_string()
                } else {
                    // For plain `require`, keep as-is (minus extension) — load path unknown
                    pfx.trim_start_matches("./").trim_start_matches('.').trim_start_matches('/') .to_string()
                };
...
fn find_ruby_block_end(lines: &[&str], start: usize) -> usize {
    ...
    for (idx, line) in lines.iter().enumerate().skip(start) {
        if re_begin.is_match(line) { depth += 1; }
        if re_end.is_match(line) { depth -= 1; if depth == 0 { return idx; } }
    }
