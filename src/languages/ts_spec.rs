...
use crate::ts_core::{load_typescript_spec, compile_queries_typescript, QueryRunner};
...
impl LanguageAnalyzer for SpecTsAnalyzer {
    ...
    fn symbols_in_file(&self, path: &str, source: &str) -> Vec<Symbol> {
        ...
        if let Some(m) = re_default.find(source) {
            ...
            out.push(Symbol { id: SymbolId::new(self.language(), path, &kind, "default", sl), name: "default".to_string(), kind, file: path.to_string(), range: TextRange { start_line: sl, end_line: el }, language: self.language().to_string() });
        }
