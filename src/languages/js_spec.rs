...
use crate::ts_core::{load_javascript_spec, compile_queries_javascript, QueryRunner};
...
impl Default for SpecJsAnalyzer {
...
impl LanguageAnalyzer for SpecJsAnalyzer {
    ...
    fn symbols_in_file(&self, path: &str, source: &str) -> Vec<Symbol> {
        ...
        for caps in self.runner.run_captures(source, &self.queries.decl) {
            if let Some(nc) = caps.iter().find(|c| c.name == "name") {
                ...
                let (sl, el) = if let Some(dc) = decl_cap { (
                    ...
                    byte_to_line(&offs, dc.end.saturating_sub(1)).max(byte_to_line(&offs, dc.start))
                ) } else { (
                    byte_to_line(&offs, nc.start),
                    byte_to_line(&offs, nc.end.saturating_sub(1)).max(byte_to_line(&offs, nc.start))
                ) };
        ...
        // Fallback: CommonJS function expressions (module.exports = function..., exports.foo = function...)
        use regex::Regex;
        fn find_block_end(src: &str, start_idx: usize) -> usize {
            // naive brace matching from first '{' after start_idx
            let bytes = src.as_bytes();
            let mut i = start_idx;
            while i < bytes.len() && bytes[i] != b'{' { i += 1; }
            if i >= bytes.len() { return start_idx; }
            let mut depth = 0i32;
            while i < bytes.len() {
                let b = bytes[i];
                if b == b'{' { depth += 1; }
                else if b == b'}' { depth -= 1; if depth == 0 { return i+1; } }
                i += 1;
            }
            bytes.len()
        }
        ...
        if let Some(m) = re_default.find(source) {
            ...
            out.push(Symbol { id: SymbolId::new("javascript", path, &kind, "default", sl), name: "default".to_string(), kind, file: path.to_string(), range: TextRange { start_line: sl, end_line: el }, language: "javascript".to_string() });
        }
        let re_named = Regex::new(r#"(?m)^\s*(?:module\.exports|exports)\.([A-Za-z_$][\w$]*)\s*=\s*function\s*\("#).unwrap();
        ...
        if let Some(m) = Regex::new(r#"(?m)^\s*module\.exports\s*=\s*\{"#).unwrap().find(source) {
            let start = m.start();
            let endb = find_block_end(source, start);
            let body = &source[start..endb];
            // method syntax foo() { }
            let re_obj_method = Regex::new(r#"(?m)\b([A-Za-z_$][\w$]*)\s*\(\s*[^\)]*\s*\)\s*\{"#).unwrap();
            ...
            // property with arrow: name: (...) => {
            let re_obj_arrow = Regex::new(r#"(?m)\b([A-Za-z_$][\w$]*)\s*:\s*\(?[^\)]*\)?\s*=>\s*\{"#).unwrap();
        ...
        // Fallback: class field arrow methods: class A { m = () => { ... } }
        // (also tolerates modifiers: public/private/protected/readonly/static)
        let re_class = Regex::new(r#"(?m)\bclass\s+[A-Za-z_$][\w$]*\s*\{"#).unwrap();
        let re_field_arrow = Regex::new(r#"(?m)(?:\s*(?:public|private|protected|readonly|static)\s+)*\s*([A-Za-z_$][\w$]*)\s*=\s*\(?[^\)]*\)?\s*=>\s*\{"#).unwrap();
        for m in re_class.captures_iter(source) {
            ...
            for cap in re_field_arrow.captures_iter(body) {
                let name = cap.get(1).unwrap().as_str();
                let s_abs = start + cap.get(0).unwrap().start();
                // find the '{' after match to compute body end
                let mut brace_from = s_abs;
                let bytes = source.as_bytes();
                while brace_from < bytes.len() && bytes[brace_from] != b'{' { brace_from += 1; }
    ...
    fn unresolved_refs(&self, path: &str, source: &str) -> Vec<UnresolvedRef> {
        ...
        for caps in self.runner.run_captures(source, &self.queries.calls) {
            ...
            if let Some(n) = name_cap {
                ...
                let qual = caps.iter().find(|c| c.name == "qual").map(|q| source[q.start..q.end].to_string());
                out.push(UnresolvedRef { name, kind: RefKind::Call, file: path.to_string(), line: ln, qualifier: qual.filter(|s| !s.is_empty()), is_method });
            }
        }
        // Fallback: optional chaining calls like obj?.m()
        use regex::Regex;
        let re_opt_member = Regex::new(r#"([A-Za-z_$][\w$\.]+)\?\.\s*([A-Za-z_$][\w$]*)\s*\("#).unwrap();
        for cap in re_opt_member.captures_iter(source) {
            ...
            out.push(UnresolvedRef { name, kind: RefKind::Call, file: path.to_string(), line: ln, qualifier: Some(q), is_method: true });
        }
        // Fallback: optional chaining on identifier: foo?.()
        ...
        // Fallback: obj.func?.()
        let re_opt_member2 = Regex::new(r#"([A-Za-z_$][\w$\.]+)\.([A-Za-z_$][\w$]*)\s*\?\.\s*\("#).unwrap();
    ...
    fn imports_in_file(&self, path: &str, source: &str) -> std::collections::HashMap<String, String> {
        ...
        let re_export_named = Regex::new(r#"(?m)^\s*export\s*\{([^}]+)\}\s*from\s*['\"]([^'\"]+)['\"]"#).unwrap();
        let re_export_all = Regex::new(r#"(?m)^\s*export\s*\*\s*from\s*['\"]([^'\"]+)['\"]"#).unwrap();
        ...
        for cap in re_export_named.captures_iter(source) {
            ...
            if let Some(norm) = normalize_es_module_path(path, raw) {
                for seg in inner.split(',') {
                    let seg = seg.trim(); if seg.is_empty() { continue; }
                    if let Some((orig, alias)) = seg.split_once(" as ") {
                        let o = orig.trim(); let a = alias.trim();
