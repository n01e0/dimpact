...
"#;
    ...
    #[test]
    fn go_hard_case_fixture_v041_generics_chained_embedded_receiver() {
        let src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
        ...
        let syms = ana.symbols_in_file("pkg/hard_v041.go", src);
        assert!(syms.iter().any(|s| {
            s.name == "Chain"
                && matches!(s.kind, SymbolKind::Function)
                && s.id.0 == "go:pkg/hard_v041.go:fn:Chain:20"
        }));
        assert!(syms.iter().any(|s| {
            s.name == "Execute"
                && matches!(s.kind, SymbolKind::Method)
                && s.id.0 == "go:pkg/hard_v041.go:method:Execute:24"
        }));

        let refs = ana.unresolved_refs("pkg/hard_v041.go", src);
        assert!(refs.iter().any(|r| {
            r.name == "Trace" && r.qualifier.as_deref() == Some("s") && r.is_method && r.line == 25
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Run"
                && r.qualifier.as_deref() == Some("s.next")
                && r.is_method
                && r.line == 27
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Chain" && r.qualifier.is_none() && !r.is_method && r.line == 26
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Background"
    ...
    #[test]
    fn go_hard_case_fixture_interface_method_value_generic_receiver() {
        let src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/go/analyzer_hard_cases_method_value.go"
        ));
        let ana = SpecGoAnalyzer::new();

        let syms = ana.symbols_in_file("pkg/hard_method_value.go", src);
        assert!(syms.iter().any(|s| {
            s.name == "Execute"
                && matches!(s.kind, SymbolKind::Method)
                && s.id.0 == "go:pkg/hard_method_value.go:method:Execute:13"
        }));

        let refs = ana.unresolved_refs("pkg/hard_method_value.go", src);
        assert!(refs.iter().any(|r| {
            r.name == "invoke" && r.qualifier.is_none() && !r.is_method && r.line == 16
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Run"
                && r.qualifier.as_deref() == Some("s.next")
    ...
    #[test]
    fn go_hard_case_fixture_interface_dispatch_method_value_generic_receiver_v2() {
        let src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/go/analyzer_hard_cases_interface_dispatch_method_value_generic_receiver_v2.go"
        ));
        let ana = SpecGoAnalyzer::new();

        let syms = ana.symbols_in_file("pkg/hard_v2_go_dispatch.go", src);
        assert!(syms.iter().any(|s| {
            s.name == "callInner"
                && matches!(s.kind, SymbolKind::Method)
                && s.id.0 == "go:pkg/hard_v2_go_dispatch.go:method:callInner:13"
        }));
        assert!(syms.iter().any(|s| {
            s.name == "Execute"
                && matches!(s.kind, SymbolKind::Method)
                && s.id.0 == "go:pkg/hard_v2_go_dispatch.go:method:Execute:17"
        }));

        let refs = ana.unresolved_refs("pkg/hard_v2_go_dispatch.go", src);
        assert!(refs.iter().any(|r| {
            r.name == "Handle"
                && r.qualifier.as_deref() == Some("p.inner")
                && r.is_method
                && r.line == 14
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Handle"
                && r.qualifier.as_deref() == Some("h")
                && r.is_method
                && r.line == 20
        }));
        assert!(refs.iter().any(|r| {
            r.name == "methodValue" && r.qualifier.is_none() && !r.is_method && r.line == 24
        }));
        assert!(refs.iter().any(|r| {
            r.name == "runner" && r.qualifier.is_none() && !r.is_method && r.line == 28
        }));
        assert!(refs.iter().any(|r| {
            r.name == "bridge" && r.qualifier.is_none() && !r.is_method && r.line == 32
        }));
        assert!(refs.iter().any(|r| {
            r.name == "Background"
                && r.qualifier.as_deref() == Some("context")
