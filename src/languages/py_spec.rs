...
"#;
    ...
    #[test]
    fn python_hard_case_fixture_monkeypatch_metaclass_protocol_v4() {
        let src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
        ...
        let refs = ana.unresolved_refs("pkg/dynamic_protocol_meta_v4.py", src);
        let refs_dbg: Vec<_> = refs
            .iter()
            .map(|r| match &r.qualifier {
                Some(q) => format!("{}@{}[{}]/{}", r.name, r.line, q, r.is_method),
        ...
        assert!(
            refs.iter()
                .any(|r| r.name == "instrument" && r.qualifier.is_none() && !r.is_method),
            "refs={refs_dbg:?}"
        );
        assert!(
            refs.iter()
        ...
        assert!(
            refs.iter()
                .any(|r| r.name == "patched_process" && r.qualifier.is_none() && !r.is_method),
            "refs={refs_dbg:?}"
        );
        assert!(
            refs.iter().any(|r| {
                r.name == "process" && r.qualifier.as_deref() == Some("Worker") && r.is_method
            }),
            "refs={refs_dbg:?}"
        );
        assert!(
            refs.iter().any(|r| {
                r.name == "process" && r.qualifier.as_deref() == Some("target") && r.is_method
            }),
            "refs={refs_dbg:?}"
        );
        assert!(
            refs.iter().any(|r| {
                r.name == "process"
                    && r.qualifier.as_deref() == Some("SupportsProcess")
                    && r.is_method
            }),
            "refs={refs_dbg:?}"
        );
        assert!(
            refs.iter().any(|r| {
                r.name == "build_worker"
                    && r.qualifier.as_deref() == Some("ServiceFactory")
                    && r.is_method
            }),
            "refs={refs_dbg:?}"
        );
        ...
        let im = ana.imports_in_file("pkg/dynamic_protocol_meta_v4.py", src);
        assert_eq!(
            im.get("wraps").map(String::as_str),
            Some("functools::wraps")
        );
