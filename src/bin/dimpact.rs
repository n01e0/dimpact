...
fn print_impact_output(
    ...
    match fmt {
        OutputFormat::Json => {
            let rendered = ImpactOutputRendered {
                output: out,
                confidence_filter,
            };
...
fn resolve_schema_profile_for_command(
    fmt: OutputFormat,
...
fn resolve_schema_profile_for_target(
    ...
    resolve_schema_profile_for_command(fmt, command)
}
...
struct Tier2SemanticEvidence {
    ...
    param_to_return_flow_count: u8,
    tail_param_impact_count: u8,
}

impl Tier2SemanticEvidence {
    fn semantic_support_rank(&self) -> u8 {
        self.param_to_return_flow_count
            .saturating_add(self.tail_param_impact_count)
    }
}
...
fn collect_rust_tier2_semantic_evidence(
    ...
    let mut param_to_return_flow_count = 0usize;
    let mut tail_param_impact_ids = std::collections::BTreeSet::new();
    for flow in summary
        .flows
        .iter()
        .filter(|flow| param_input_ids.contains(&flow.input_node_id))
    {
        let mut reaches_tail = false;
        for node_id in &flow.impacted_node_ids {
            if node_line_by_id
                .get(node_id)
                .is_some_and(|line| *line >= tail_line_floor)
            {
                reaches_tail = true;
                tail_param_impact_ids.insert(node_id.clone());
            }
        }
        if reaches_tail {
            param_to_return_flow_count += 1;
        }
    }
    let param_to_return_flow = param_to_return_flow_count > 0;

    Tier2SemanticEvidence {
        param_to_return_flow,
        param_to_return_flow_count: u8::try_from(param_to_return_flow_count).unwrap_or(u8::MAX),
        tail_param_impact_count: u8::try_from(tail_param_impact_ids.len()).unwrap_or(u8::MAX),
    }
}
    ...
    a.scoring
        ...
        .then_with(|| {
            b.scoring
                .score_tuple
                .semantic_support_rank
                .cmp(&a.scoring.score_tuple.semantic_support_rank)
        })
        .then_with(|| {
            b.scoring
                .score_tuple
        ...
        if seeds.is_empty() {
            ...
            if with_pdg || with_propagation {
                ...
                match fmt {
                    ...
                    OutputFormat::Dot | OutputFormat::Html => {
                        anyhow::bail!("format not supported for 'impact --per-seed': use json|yaml")
                    }
                }
                return Ok(());
            }
...
"#,
    ...
    #[test]
    fn bounded_slice_plan_allows_distinct_continuation_bridge_families_to_coexist() {
        let seed = test_symbol("rust:main.rs:fn:caller:1", "caller", "main.rs", 1);
        let alias_wrapper = test_symbol(
            "rust:alias_wrapper.rs:fn:wrap_alias:3",
            "wrap_alias",
            "alias_wrapper.rs",
            3,
        );
        let return_wrapper = test_symbol(
            "rust:return_wrapper.rs:fn:wrap_return:3",
            "wrap_return",
            "return_wrapper.rs",
            3,
        );
        let value = test_symbol("rust:value.rs:fn:make:1", "make", "value.rs", 1);
        let leaf = test_symbol("rust:leaf.rs:fn:leaf:1", "leaf", "leaf.rs", 1);
        let out = test_symbol("rust:out.rs:fn:alias_out:1", "alias_out", "out.rs", 1);
        let final_leaf = test_symbol(
            "rust:final_leaf.rs:fn:finish:1",
            "finish",
            "final_leaf.rs",
            1,
        );
        ...
        let refs = vec![
            call_ref(
                seed.id.0.as_str(),
                alias_wrapper.id.0.as_str(),
                "main.rs",
                4,
            ),
            call_ref(
                seed.id.0.as_str(),
                return_wrapper.id.0.as_str(),
                "main.rs",
                5,
            ),
            call_ref(
            ...
            call_ref(value.id.0.as_str(), out.id.0.as_str(), "value.rs", 3),
            call_ref(leaf.id.0.as_str(), final_leaf.id.0.as_str(), "leaf.rs", 4),
        ];
        ...
        assert_eq!(
            ...
            vec![ImpactSliceReasonMetadata {
                ...
                tier: 2,
                kind: ImpactSliceReasonKind::BridgeCompletionFile,
                via_symbol_id: Some("rust:alias_wrapper.rs:fn:wrap_alias:3".to_string()),
                via_path: Some("alias_wrapper.rs".to_string()),
                bridge_kind: Some(ImpactSliceBridgeKind::BoundaryAliasContinuation),
                scoring: Some(test_tier2_scoring(
                    ImpactSliceCandidateLane::AliasContinuation,
                    vec![
        ...
        let final_leaf_file = slice_selection_file(&plan.slice_selection, "final_leaf.rs");
        assert_eq!(
            ...
            vec![ImpactSliceReasonMetadata {
                ...
                tier: 3,
                kind: ImpactSliceReasonKind::BridgeContinuationFile,
                via_symbol_id: Some("rust:leaf.rs:fn:leaf:1".to_string()),
                via_path: Some("leaf.rs".to_string()),
                bridge_kind: Some(ImpactSliceBridgeKind::WrapperReturn),
                scoring: Some(test_tier2_scoring(
                    ImpactSliceCandidateLane::ReturnContinuation,
                    vec![
        ...
        assert!(
            plan.cache_update_paths.contains(&"out.rs".to_string())
                && plan
                    .cache_update_paths
                    .contains(&"final_leaf.rs".to_string())
        );
        assert!(plan.slice_selection.pruned_candidates.is_empty());
    }

    #[test]
    fn bounded_slice_plan_limits_continuation_budget_per_bridge_family() {
        let seed = test_symbol("rust:main.rs:fn:caller:1", "caller", "main.rs", 1);
        ...
        let final_a = test_symbol("rust:a_final.rs:fn:final_a:1", "final_a", "a_final.rs", 1);
        let final_b = test_symbol("rust:b_final.rs:fn:final_b:1", "final_b", "b_final.rs", 1);
        let index = SymbolIndex::build(vec![
            seed.clone(),
            wrap_a.clone(),
            wrap_b.clone(),
        ...
        let refs = vec![
            ...
            call_ref(leaf_a.id.0.as_str(), final_a.id.0.as_str(), "a_leaf.rs", 4),
            call_ref(leaf_b.id.0.as_str(), final_b.id.0.as_str(), "b_leaf.rs", 4),
        ];
        ...
        let a_final_file = slice_selection_file(&plan.slice_selection, "a_final.rs");
        assert_eq!(
            ...
            vec![ImpactSliceReasonMetadata {
                ...
                tier: 3,
                kind: ImpactSliceReasonKind::BridgeContinuationFile,
                via_symbol_id: Some("rust:a_leaf.rs:fn:leaf_a:1".to_string()),
                via_path: Some("a_leaf.rs".to_string()),
                bridge_kind: Some(ImpactSliceBridgeKind::WrapperReturn),
                scoring: Some(test_tier2_scoring(
                    ImpactSliceCandidateLane::ReturnContinuation,
                    vec![
            ...
            plan.slice_selection
                ...
                .any(|candidate| {
                    candidate.path == "b_final.rs"
                        ...
                        && candidate.via_symbol_id.as_deref() == Some("rust:b_leaf.rs:fn:leaf_b:1")
                        && candidate.via_path.as_deref() == Some("b_leaf.rs")
                }),
            "expected second wrapper-return continuation family member to be pruned once the per-family budget is spent: {:#?}",
            plan.slice_selection.pruned_candidates
        );
    }
    ...
    #[test]
    #[serial]
    fn bounded_slice_plan_prefers_stronger_rust_semantic_support_over_later_callsite_hint() {
        let dir = TempDir::new().expect("tempdir");
        let root = dir.path();
        fs::write(
            root.join("steady.rs"),
            "pub fn carry(input: i32) -> i32 {\n    let forwarded = input;\n    let settled = forwarded;\n    settled + forwarded\n}\n",
        )
        .unwrap();
        fs::write(
            root.join("plain.rs"),
        ...
        let steady = dimpact::Symbol {
            id: SymbolId("rust:steady.rs:fn:carry:1".to_string()),
            name: "carry".to_string(),
            kind: dimpact::SymbolKind::Function,
            file: "steady.rs".to_string(),
            range: dimpact::TextRange {
                start_line: 1,
                end_line: 5,
            },
        ...
        let plain = dimpact::Symbol {
            id: SymbolId("rust:plain.rs:fn:carry:1".to_string()),
            name: "carry".to_string(),
            kind: dimpact::SymbolKind::Function,
            file: "plain.rs".to_string(),
            range: dimpact::TextRange {
                start_line: 1,
                end_line: 3,
            },
        ...
        let refs = vec![
            call_ref(seed.id.0.as_str(), wrapper.id.0.as_str(), "main.rs", 7),
            call_ref(wrapper.id.0.as_str(), steady.id.0.as_str(), "wrapper.rs", 5),
            call_ref(wrapper.id.0.as_str(), plain.id.0.as_str(), "wrapper.rs", 6),
        ];
        ...
        let steady_file = slice_selection_file(&plan.slice_selection, "steady.rs");
        assert_eq!(
            steady_file.reasons,
            vec![ImpactSliceReasonMetadata {
                seed_symbol_id: seed.id.0.clone(),
                ...
                scoring: Some(ImpactSliceCandidateScoringSummary {
                    ...
                    score_tuple: ImpactSliceScoreTuple {
                        ...
                        call_position_rank: 5,
                        lexical_tiebreak: "steady.rs".to_string(),
                    },
                    support: Some(ImpactSliceCandidateSupportMetadata {
                        local_dfg_support: true,
                        ..ImpactSliceCandidateSupportMetadata::default()
            ...
            vec![dimpact::ImpactSlicePrunedCandidate {
                ...
                scoring: Some(ImpactSliceCandidateScoringSummary {
                    ...
                    score_tuple: ImpactSliceScoreTuple {
                        ...
                        semantic_support_rank: 2,
                        call_position_rank: 6,
                        lexical_tiebreak: "plain.rs".to_string(),
                    },
                    support: Some(ImpactSliceCandidateSupportMetadata {
                        local_dfg_support: true,
                        ..ImpactSliceCandidateSupportMetadata::default()
